...
namespace adore
{
	namespace mad
	{
        int CoordinateConversion::LatLonToUTMXY (double lat, double lon, int zone, double& x, double& y)
        {
            if ( (zone < 1) || (zone > 60) )
        ...
        void CoordinateConversion::UTMXYToLatLon (double x, double y, int zone, bool southhemi, double& lat, double& lon) //zone - The UTM zone in which the point lies, southhemi - True if the point is in the southern hemisphere; false otherwise.
        {
        ...
        void CoordinateConversion::UTMXYToLatLonDegree (double x, double y, int zone, bool southhemi, double& lat, double& lon) //zone - The UTM zone in which the point lies, southhemi - True if the point is in the southern hemisphere; false otherwise.
        {
            CoordinateConversion::UTMXYToLatLon (x, y, zone, southhemi, lat, lon) ;
            lat = RadToDeg (lat);
            lon = RadToDeg (lon);
        }
        double CoordinateConversion::DegToRad(double deg)
        {
            return (deg / 180.00 * PI);
        }
        double CoordinateConversion::RadToDeg(double rad)
        {
            return (rad / PI * 180.00);
        }
        double CoordinateConversion::twoPIrange(double rad)
        {
            double eps = 1.0e-9;
        ...
        double CoordinateConversion::ArcLengthOfMeridian(double phi)  //returns the ellipsoidal distance of the point from the equator, in meters
        {
        ...
        double CoordinateConversion::UTMCentralMeridian(int zone)      //returns the central meridian for the given UTM zone, in radians. Range of the central meridian is the radian equivalent of [-177,+177].
        {
            double cmeridian;
            cmeridian = DegToRad(-183.0 + ((double)zone * 6.0));  
            return cmeridian;
        }        
        double CoordinateConversion::FootpointLatitude(double y)		//returns the footpoint latitude, in radians
        {
        ...
        void CoordinateConversion::MapLatLonToXY (double phi, double lambda, double lambda0, double &x, double &y)
        {
        ...
        void CoordinateConversion::MapXYToLatLon (double x, double y, double lambda0, double& phi, double& lambda)
        {
